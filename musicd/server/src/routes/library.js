...
// v1.1.0.82 — Saved Focus combinations.
//
// CRUD for the user's named focus presets. Storage is server-side
// (sqlite saved_focuses table) so they survive client reloads and
// follow the user across devices on the same MusicD instance.
//
// Hard cap at 20 rows; this is enforced at the create endpoint
// (v82 spec — "20 max for now"). Updates and deletes don't need
// the cap check because they don't grow the row count.

const SAVED_FOCUS_MAX_ROWS    = 20;
const SAVED_FOCUS_MAX_NAME    = 60;
const SAVED_FOCUS_MAX_JSON_KB = 16;  // 16KB is huge for a few Sets

// Helper: serialise picks back to client-friendly shape. The hook
// stores Sets which JSON.stringify renders as `{}` — we store a
// pre-serialised object on the client side already (see useFocusState
// queryString builder). What we save on the server is whatever the
// client posted; what we return is whatever was saved.
function serialisePickRow(row) {
  if (!row) return null;
  return {
    id: row.id,
    name: row.name,
...
// GET /api/library/focus/saved
//   → { rows: [{ id, name, picks, created_at, updated_at }, ...] }
router.get('/focus/saved', (req, res) => {
  try {
    const database = db.get();
    const rows = database.prepare(`
      SELECT id, name, picks_json, created_at, updated_at
      FROM saved_focuses
      ORDER BY name COLLATE NOCASE ASC
    `).all();
    res.json({ rows: rows.map(serialisePickRow) });
  } catch (e) {
    console.error('[saved-focus] list failed:', e);
    res.status(500).json({ error: e.message });
  }
});

// POST /api/library/focus/saved
//   body: { name: string, picks: object }
//   → { row: {...} } | { error }
router.post('/focus/saved', (req, res) => {
  const name = String(req.body?.name || '').trim();
  const picks = req.body?.picks;
  if (!name) return res.status(400).json({ error: 'name is required' });
  if (name.length > SAVED_FOCUS_MAX_NAME) {
    return res.status(400).json({ error: `name too long (max ${SAVED_FOCUS_MAX_NAME} chars)` });
  }
  ...
  try {
    ...
    return res.status(400).json({ error: 'picks could not be serialised' });
  }
  if (picksJson.length > SAVED_FOCUS_MAX_JSON_KB * 1024) {
    return res.status(400).json({ error: `picks too large (max ${SAVED_FOCUS_MAX_JSON_KB} KB)` });
  }

  try {
    const database = db.get();
    // Cap check
    const count = database.prepare('SELECT COUNT(*) AS c FROM saved_focuses').get().c;
    if (count >= SAVED_FOCUS_MAX_ROWS) {
      return res.status(400).json({
        error: `Max ${SAVED_FOCUS_MAX_ROWS} saved focuses. Delete one first.`,
      });
    }
    // Unique name check (better explicit message than the SQLite
    // UNIQUE constraint hit)
    const existing = database.prepare('SELECT id FROM saved_focuses WHERE name = ?').get(name);
    if (existing) {
      return res.status(409).json({ error: 'A saved focus with that name already exists.' });
    }
    const result = database.prepare(`
      INSERT INTO saved_focuses (name, picks_json) VALUES (?, ?)
    `).run(name, picksJson);
    const row = database.prepare(`
      SELECT id, name, picks_json, created_at, updated_at
      FROM saved_focuses WHERE id = ?
    `).get(result.lastInsertRowid);
    res.json({ row: serialisePickRow(row) });
  } catch (e) {
    console.error('[saved-focus] create failed:', e);
    res.status(500).json({ error: e.message });
  }
});

// PUT /api/library/focus/saved/:id
//   body: { name?: string, picks?: object }
//   → { row: {...} }
//
// Used by the "Update Late Night Jazz" dirty-state button — sends
// the new picks under the same name. Also handles rename if the
// client sends a `name` field (future use; v82 UI doesn't expose
// rename yet, but the endpoint is here for v83+).
router.put('/focus/saved/:id', (req, res) => {
  const id = parseInt(req.params.id, 10);
  if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid id' });
  const name = req.body?.name !== undefined ? String(req.body.name).trim() : undefined;
  const picks = req.body?.picks;
  if (name === undefined && picks === undefined) {
    return res.status(400).json({ error: 'name or picks required' });
  }
  if (name !== undefined && (!name || name.length > SAVED_FOCUS_MAX_NAME)) {
  ...
  if (picks !== undefined) {
    ...
    if (picksJson.length > SAVED_FOCUS_MAX_JSON_KB * 1024) {
      return res.status(400).json({ error: 'picks too large' });
    }
  }

  try {
    const database = db.get();
    const existing = database.prepare('SELECT id, name FROM saved_focuses WHERE id = ?').get(id);
    if (!existing) return res.status(404).json({ error: 'not found' });

    // Rename collision check
    if (name !== undefined && name !== existing.name) {
      const collision = database.prepare('SELECT id FROM saved_focuses WHERE name = ? AND id != ?').get(name, id);
      if (collision) return res.status(409).json({ error: 'name already used' });
    }

    const sets = [];
    const params = [];
    if (name !== undefined) { sets.push('name = ?'); params.push(name); }
    if (picksJson !== undefined) { sets.push('picks_json = ?'); params.push(picksJson); }
    sets.push("updated_at = unixepoch()");
    params.push(id);
    database.prepare(`UPDATE saved_focuses SET ${sets.join(', ')} WHERE id = ?`).run(...params);
    ...
    console.error('[saved-focus] update failed:', e);
    res.status(500).json({ error: e.message });
  }
});

// DELETE /api/library/focus/saved
//   body: { ids: [1, 2, 3] }
//   → { deleted: count }
//
// Bulk delete by id list. UI shows a tickbox + trash flow and the
// server takes the resulting id list in one POST so the user
// experience is "tick three, tap trash, confirm" rather than three
// network round-trips. Idempotent — non-existent ids are silently
// skipped.
router.delete('/focus/saved', (req, res) => {
  const ids = Array.isArray(req.body?.ids) ? req.body.ids : null;
  if (!ids || ids.length === 0) {
    return res.status(400).json({ error: 'ids array required' });
  }
  // Validate every entry is an integer
  ...
  try {
    ...
    const result = database.prepare(`
      DELETE FROM saved_focuses WHERE id IN (${placeholders})
    `).run(...intIds);
    res.json({ deleted: result.changes || 0 });
  } catch (e) {
    console.error('[saved-focus] delete failed:', e);
    res.status(500).json({ error: e.message });
  }
});
